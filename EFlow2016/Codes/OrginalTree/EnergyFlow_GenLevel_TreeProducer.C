...
void  EnergyFlow_GenLevel_TreeProducer()

{
    ...
    static const Int_t ftyp =5 ;// Data and MC
    string fname[5]={"herwigpp","pythia8_Monash","pythia8_MBR","epos","qgsjetII"};
    ...
    string filenames[5] ={"root://eoscms.cern.ch//eos/cms/store/user/zdemirog/13TeV/MC/MinBias_TuneEE5C_13TeV-herwigpp_MagnetOff_trees.root",
	...
	sprintf(title,"EFlow_GenLevel_%s_tree_0Tesla.root",fname[f].c_str());
	...
	fOutFile[f]->Write();
	delete fOutFile[f];
	cout<<"Filling tree and write in root file with "<<TotNofEventGen[f]<<" events"<<" for file : "<<fname[f].c_str()<<endl;
    }//File
