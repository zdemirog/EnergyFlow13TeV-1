...
#include "MinBias_Herwig_13TeV-pythia8.h"
using namespace std;
...
void  EnergyFlow_GenLevel_Herwig0T_TreeProducer()

{
    ...
    vector<float> Gen;
    vector<float> SignalGen;
    vector<float> SignalVetoGen;
	...
	sprintf(title,"EFlow_GenLevel_%s_0Tesla_SDEnhanced_FromBenoit_Full.root",readfilesname.c_str());
	...
	sprintf(title,"EFlow");
        ...
        EnergyFlow_Gen[f]->Branch("SignalGen",&SignalGen);
        EnergyFlow_Gen[f]->Branch("SignalVetoGen",&SignalVetoGen);
	...
	//------- File Number ---------//                                                                                                                                                
        for (int fn=0; fn < fnumber ; fn++){
            // --- open file in EOS                                                                                                                                                      
            char Fname[999];
            sprintf(Fname,"Herwig_0T_20%d5A_ReConst_Herwig2015AMagnetOff.root",(fn+1));
            string sFname(Fname);
       ...
       // }
		...
		for (int i =0; i<nEtaBins;i++) {
            ...
            GenEtaSums[i]=0.;
            SignalGenEtaSums[i]=0.; //dsdsdsdsdsdsd
            SignalVetoGenEtaSums[i]=0.; //sdsdssds
            GenEtaSums_1GeV[i]=0.;
            GenEtaSums_2GeV[i]=0.;
        ...
        if((StableParticleEnergyCutPlus && !StableParticleEnergyCutMinus) || (!StableParticleEnergyCutPlus && StableParticleEnergyCutMinus)){
            ...
            for (int k=0;k<nEtaBins;k++){
                ...
                SignalGen.push_back(SignalGenEtaSums[k]);
                SignalVetoGen.push_back(SignalVetoGenEtaSums[k]);
