...
void  EnergyFlow_DetLevel_Herwig0T_HFANDSelection_TreeProducer()

{
	...
	sprintf(title,"EFlow_DetLevel_%s_AllCollect_HFANDSelection_WithLastRing.root",readfilesname.c_str());
	...
	//------- File Number ---------//                                                                                                                                                
	    ...
	    // ----------------------- Cut-------------------------//
            ...
            lumimax =311;
            //emin = 5.;
            emin = 4.; // as to HFAnd Selection
            Etabnd = 5.191;//Max Eta for CMS
            cmseta = 6.6;//Max Eta for CMS
            etamin =3.139;
            //etamax =4.889;//event select
            etamax =5.191;//event select with last ring
            etamaxMinus =-3.139;
            //etaminMinus =-4.889;//event select
            etaminMinus =-5.191;//event select with last ring
 ...
 */
			...
			if (EnergyCutTowerMinus && EnergyCutTowerPlus){
				...
				selcetetabin=getBin(caltwr.Eta(),EtaBins,nEtaBins);
                          	if(abs(caltwr.Eta()) >=etamin && abs(caltwr.Eta()) <=Etabnd) HCALTowerEtaSums[selcetetabin]= HCALTowerEtaSums[selcetetabin] + (caltwr.E())*Norm;//HF MC Norm
