...
#include "DataZeroBias1_13TeV_0T.h"
using namespace std;
...
void  EnergyFlow_DetLevel_DataZeroBias1AllBx_TreeProducer_with5p5GeV()

{
	...
	sprintf(title,"EFlow_DetLevel_%s_HFOR_r247324_tree_JINSTbinning_with5p5GeV.root",readfilesname.c_str());
		...
		EnergyCutTower=true;
	      ...
	      if (EnergyCutTower){
			    ...
			    for(unsigned long j=0; j<HFRecHitenergy->size(); j++) {
                    //if((abs(HFRecHiteta->at(j))>=etamin && abs(HFRecHiteta->at(j))<=Etabnd) && (HFRecHitenergy->at(j))*Norm >2.0){
