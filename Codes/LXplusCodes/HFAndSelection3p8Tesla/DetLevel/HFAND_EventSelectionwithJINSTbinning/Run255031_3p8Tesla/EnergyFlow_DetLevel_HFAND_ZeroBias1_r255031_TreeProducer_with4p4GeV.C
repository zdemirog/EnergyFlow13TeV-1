...
void  EnergyFlow_DetLevel_HFAND_ZeroBias1_r255031_TreeProducer_with4p4GeV()
{
  ...
  bool save =true;
  string filenames ="";
  string energyname[15]={"RecHitHCAL","RecHitHF","RecHitECAL","RecHit","CaloTower","HCALTower",
  ...
  for (int f=0; f<ftyp; f++){
    ...
    sprintf(title,"EFlow_DetLevel_%s_HFAND_r255031_tree_3p8Tesla_with4p4GeV.root",readfilesname.c_str());
	...
	if (filetype =="DATA" && run==255031) RunCut=true;
	...
	if(RunCut){
	  ...
	  if(TrigerPass){
		...
		CastorTowerEtaSums[selcetetabin]= CastorTowerEtaSums[selcetetabin] + castwr.E();
	      ...
	      float totenergyrechit,totenergypfclusters,totenergypfcandidate;
